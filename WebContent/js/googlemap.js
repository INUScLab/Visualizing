...
// if (y == 0)
	...
	// console.log(leakcount , freezecount, fatcount , breakcount , reversecount
	// , absencecount)
...
function drawConsumerReport( addressArray ){
	...
	for ( var i = 0 ; i < summaryReportList.length ; i ++ ) {
		...
		if(summaryReportList[i].gu == gu && summaryReportList[i].dong == dong && summaryReportList[i].detail == detail ) {
			...
			// String형 변수 int로 변환
			summaryReportList[i].consumed *= 1;
			summaryReportList[i].predicted *=1;
			// 3.요약 report 사용량,예측량, 일주일 평균 , 지역 평균 그래프 그리기.
			drawColumn(summaryReportList[i].consumed,
					summaryReportList[i].predicted,
					sum_weeklyConsumption/7,
					dongConsumption / dongCount);
			...
			console.log(summaryReportList[i]);
			document.getElementById('blockLeak').innerHTML = summaryReportList[i].leak + "건";
			document.getElementById('blockFreezed').innerHTML = summaryReportList[i].freezed + "건";
			document.getElementById('blockFat').innerHTML = summaryReportList[i].fat + "건";
			document.getElementById('blockBreakage').innerHTML = summaryReportList[i].breakage + "건";
			document.getElementById('blockReverse').innerHTML = summaryReportList[i].reverse + "건";
			document.getElementById('blockAbsence').innerHTML = summaryReportList[i].absence + "건";
		}
	}
...
/*
 * function drawServiceFrequency( count_leak , count_absence , count_freezed ,
 * count_reverse , count_fat , count_breakage ) { var data =
 * google.visualization.arrayToDataTable([ ["Element", "Density", { role:
 * "style" } ], ["누수", count_leak, "color: blue"], ["부재중", count_absence,
 * "color: blue"], ["동파", count_freezed, "color: blue"], ["역류", count_reverse,
 * "color: blue"], ["비만", count_fat, "color: blue"], ["파손", count_breakage,
 * "color: blue"] ]);
 * 
 ...
 * 2]);
 * 
 * var options = { title: "지번 달 부가서비스 발생 횟수", height: 200, chartArea: {'width':
 * '80%', 'height': '80%'}, bar: {groupWidth: "40%"}, legend: { position: "none" }, };
 * var chart = new
 * google.visualization.ColumnChart(document.getElementById("info_service"));
 * chart.draw(view, options);
 *  }
 */
// 동 요약 리포트
...
function drawDongSummaryReport(addressArray) {
	...
	// 1.주소칸
	document.getElementById('modal-title').innerHTML = incheon+ ' ' + gu + ' ' + dong ; // 주소
																						// 출력
	...
	// console.log(sum_consumed + " " + sum_predicted + " " +
	// sum_weeklyConsumption/7 + "입니다");
	
	// 3.요약 report 사용량,예측량, 일주일 평균 , 지역 평균 그래프 그리기.
	drawColumn(sum_consumed, sum_predicted, sum_weeklyConsumption/7, sum_consumed_gu / dongCount);
	
	// 4.요약 report history 그래프 그리기.
	drawHistory(sum_day7 , sum_day6 , sum_day5 , sum_day4 , sum_day3 , sum_day2 , sum_day1 , sum_weeklyConsumption/7 );
	
	// drawServiceFrequency(leakFrequency, absenceFrequency, freezedFrequency,
	// reverseFrequency, fatFrequency, breakageFrequency);
 ...
 * Klinggaard (twitter@bklinggaard) @demo: http://dinbror.dk/bpopup @version:
 * 0.9.4.min
 * ================================================================================
 */
