...
<script type="text/javascript">
	...
	// 랭킹 그래프 로드
	google.charts.setOnLoadCallback(drawUseRankChart);
	google.charts.setOnLoadCallback(drawUsePredictRankChart);


	// 상위랭킹 한달간 데이터 그래프 로드
	google.charts.setOnLoadCallback(drawUpperUseRankChart);
	google.charts.setOnLoadCallback(drawUpperUsePredictRankChart);

	// 사용량 랭킹 그래프
	function drawUseRankChart() {
		...
		var data = google.visualization.arrayToDataTable([
         ...
         <%for(int i=0;i<Integer.parseInt(topnum);i++){ %>
         	['<%=rd_data.getConsumed_rank().get(i).get(0)%>', <%=rd_data.getConsumed_rank().get(i).get(1)%>, '<%=color[i]%>'],
         <%}%>
		...
		var chart = new google.visualization.PieChart(document.getElementById('wateruse_rank_chart_div'));
		chart.draw(data, options);
	}
	
	// 사용량 예측량 비교 그래프
	function drawUsePredictRankChart() {
		...
		var data = google.visualization.arrayToDataTable([
         ['Element', 'Density', { role: 'style' }],
         <%for(int i=0;i<Integer.parseInt(topnum);i++){ %>
      	['<%=rd_data.getPredictedConsumed_rank().get(i).get(0)%>', <%=rd_data.getPredictedConsumed_rank().get(i).get(1)%>, '<%=color[i]%>'],
      	<%}%>
		...
		// Instantiate and draw the chart
		var chart = new google.visualization.PieChart(document.getElementById('use_predict_rank_chart_div'));
		chart.draw(data, options);
	}
	...
	// 누수 상위 데이터 그래프
	function drawUpperUseRankChart() {
		...
		var data = google.visualization.arrayToDataTable([
         ['Date', 
        	 <% for(int i=0; i<Integer.parseInt(topnum);i++){%>
        	 '<%=rd_data.getConsumed_rank().get(i).get(0)%>',
        	 <%}%>],
        	 <% for(int i=0; i<rd_data.getUpper_consumed_data().get(0).size(); i++){ %>
         [ '<%=textDate2%>', <%for(int j=0;j<Integer.parseInt(topnum);j++){%><%=rd_data.getUpper_consumed_data().get(j).get(i)%>,<%}%>],
         <%// 시간 더하기
		...
		// Instantiate and draw the chart
		var chart = new google.visualization.ColumnChart(document.getElementById('wateruse_upper_chart_div'));
		chart.draw(data, options);
	}
	...
	// 동파 상위 데이터 그래프
	function drawUpperUsePredictRankChart() {
		...
		var data = google.visualization.arrayToDataTable([
         ['Date',
        	 <% for(int i=0; i<Integer.parseInt(topnum);i++){%>
        	 '<%=rd_data.getPredictedConsumed_rank().get(i).get(0)%>',
        	 <%}%>],
        	 <% for(int i=0; i<rd_data.getUpper_predicted_consumed_data().get(0).size(); i++){ %>
         [ '<%=textDate2%>', <%for(int j=0;j<Integer.parseInt(topnum);j++){%><%=rd_data.getUpper_predicted_consumed_data().get(j).get(i)%>,<%}%>],
		...
		// Instantiate and draw the chart
		var chart = new google.visualization.ColumnChart(document.getElementById('use_predict_upper_chart_div'));
		chart.draw(data, options);
	}
							...
							<section class="panel b-b-n">
								...
								<header class="m-b-lg">
									<div
										...
										<div class="col-sm-4">
											<h3 class="m-t m-b-none text-primary font-semibold">지역별 사용량 분석</h3>
								...
								<div class="row padder">
									...
									<form action="analysisWateruse.jsp" method="post" id=search_form>
										...
										<section class="panel">
											<header class="panel-heading text-primary font-semibold h5">
												<i class="fa fa-chevron-circle-right"></i> 사용량
											</header>
											<div class="row m-t m-b wrapper">
												<div class="col-lg-3">
													<section class="panel">
														<header class="panel-heading">사용량 지역별 순위</header>
														<div class="panel-body text-center" id="wateruse_rank_chart_div"></div>
													</section>
												</div>

												<div class="col-lg-9">
													<section class="panel">
														<header class="panel-heading">사용량 평균값 비교</header>
														<div class="panel-body text-center" id="wateruse_upper_chart_div"></div>
													</section>
												</div>
									...
									<div class="col-md-12">
										<section class="panel">
											<header class="panel-heading text-primary font-semibold h5">
												<i class="fa fa-chevron-circle-right"></i> 사용량 예측량 차이
											</header>
											<div class="row m-t m-b wrapper">
												<div class="col-lg-3">
													<section class="panel">
														<header class="panel-heading">사용량 예측량 차이 지역별 순위</header>
														<div class="panel-body text-center" id="use_predict_rank_chart_div"></div>
													</section>
												</div>

												<div class="col-lg-9">
													<section class="panel">
														<header class="panel-heading">사용량 예측량 차이 평균값 비교</header>
														<div class="panel-body text-center" id="use_predict_upper_chart_div"></div>
													</section>
												</div>
